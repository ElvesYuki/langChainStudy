...
当前工程为 V3.3：V3 完成工程重构，V3.1 恢复并增强第二版的内容密度，V3.2 按内容关系重新设计演讲视觉，V3.3 将演讲统一为白色与浅色主题。
...
第一章演讲稿共 22 页，支持概念点击或空格依次切换、渐进显示、键盘翻页、全屏、演讲者备注、点击底边彩虹进度线打开页码跳转，以及跳到最前和最后。

演讲页面不使用统一卡片模板。文件、流程、层级、状态、调用、职责和异常分别使用工作台、时间线、分层图、看板、控制台、泳道和分支图表达。整体采用白色画布、浅蓝灰环境和小面积强调色，深色仅保留在必要的代码片段中。模块化仅用于代码维护，不限制每页构图。
