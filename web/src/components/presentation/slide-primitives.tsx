...
const RevealStepSetterContext = createContext<((step: number) => void) | null>(null);

export function RevealProvider({ step, onStepChange, children }: { step: number; onStepChange?: (step: number) => void; children: ReactNode }) {
  return (
    <RevealContext.Provider value={step}>
      <RevealStepSetterContext.Provider value={onStepChange ?? null}>{children}</RevealStepSetterContext.Provider>
    </RevealContext.Provider>
  );
}

export function useRevealStep() {
  const step = useContext(RevealContext);
  return Number.isFinite(step) ? step : 0;
}

export function useRevealStepSetter() {
  return useContext(RevealStepSetterContext);
}
...
export function SlideCard({
  ...
  return (
    <article className={cn("h-full rounded-[1.4rem] border p-[6%] shadow-sm shadow-slate-950/4", slideTone[tone], className)}>
      {label && <span className="font-mono text-[clamp(10px,0.72vw,14px)] font-semibold tracking-[0.16em] opacity-55">{label}</span>}
      <h2 className="mt-[2%] text-[clamp(17px,1.55vw,30px)] font-semibold leading-tight tracking-[-0.025em]">{title}</h2>
      {children && <div className="mt-[4%] text-[clamp(13px,1.02vw,20px)] leading-[1.5] opacity-75">{children}</div>}
    </article>
  );
}
